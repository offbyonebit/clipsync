...
class SyncthingClient:
    ...
    def wait_until_ready(self, timeout: float = _STARTUP_WAIT) -> bool:
        ...
        Uses _STARTUP_PING_TIMEOUT (3s) per attempt instead of _API_TIMEOUT
        (10s). With 10s/attempt and a 30s window we only got 2-3 tries;
        now we get ~17 in 60s, which survives the initial DB scan on slow
        Windows storage.
        """
