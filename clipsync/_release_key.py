"""Embedded Syncthing release-signing public key (trust anchor).

ClipSync verifies the PGP signature on Syncthing's ``sha256sum.txt.asc``
before trusting the hashes it contains. The trust anchor for that
verification is the key below -- it is embedded here, NOT fetched from a
keyserver at verify-time (fetching the trust anchor over the network at
the moment of verification would let whoever controls the network also
control which key we trust, defeating the purpose).

The key and fingerprint are the official Syncthing Release Management key,
published by the Syncthing project at:

    https://syncthing.net/release-key.txt

and referenced from their security page (https://syncthing.net/security/)
as the key used to sign binary releases. Fetched 2026-07-14.

Do not edit this file unless you are intentionally rotating the trusted
release key; any change to the fingerprint must be cross-checked against an
authoritative Syncthing source.
"""

from __future__ import annotations

# Fingerprint of the key below, normalized (no spaces), as reported by
# `gpg --show-keys --fingerprint`. Verified against syncthing.net.
SYNCTHING_RELEASE_FINGERPRINT: str = "FBA2E162F2F44657B38F0309E5665F9BD5970C47"

SYNCTHING_RELEASE_KEY: str = """-----BEGIN PGP PUBLIC KEY BLOCK-----
